import sys
from PyQt5.QtWidgets import (QWidget, QToolButton, QApplication,QLabel,
...
class Explorer(QWidget):
    ...
    def makeExecuteBar(self):
        ...
        self.executeBar.returnPressed.connect(self.execute) # 执行框响应回车执行命令
...
class FileWidget(QWidget):
    ...
    def pasteItem(self):
        ...
        item.triggered.connect(self.pasteFunction)
        return item
    ...
    def pasteFunction(self):
        ...
        for i in self.buttonList:
            if i.text() == name:
                if self.clipboard['path'] == i.info['path']:
                    return
                # self.errorBox('存在同名文件或文件夹!')
                reply = QMessageBox.question(
                    self,'覆盖确认','存在同名文件或文件夹！确认覆盖吗？',
                    QMessageBox.Yes|QMessageBox.No,QMessageBox.No
                )
                if reply == QMessageBox.Yes:
                    if mydisk.is_dir(i.info['path']):
                        mydisk.delete_dir(i.info['path'])
                    else:
                        mydisk.delete_file(i.info['path'])
                    break
                else:
                    return
...
class AttributeBox(QWidget):
    ...
    def __init__(self, attribute: int = 0):
        ...
        ly.addWidget(self.confirmButton, 1, 1)
...
class deleteConfirmDialog(QWidget):
    ...
    def __init__(self, master: MyButton):
        super().__init__()
        self.label = QLabel()
        self.label.setText('确认要删除吗？')
        self.confirmButton = QPushButton()
        self.confirmButton.setText('确认')
        self.confirmButton.clicked.connect(master.deleteFunction)
