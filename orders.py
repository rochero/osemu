...
def parser(order: str) -> str:
    ...
    if order == 'mkdir':
        if len(args) != 1:
            return ['display', '参数错误!']
        if args[0].count('$') != 0 or args[0].count('.') != 0 or args[0].count(
                '/') != 0:
            return ['display', '名称错误!']
    ...
    if order == 'create':
        ...
        return [
            'display',
            __create_file(path, args[0], args[1], ' '.join(args[2:]))
        ]
    if order == 'delete':
        if len(args) != 1:
            return ['display', '参数错误!']
...
def __to_time(time: bytes) -> str:
    ...
    hour = time[3]
    minute = time[4]
    return '2{0:0>3}/{1:02}/{2:02}  {3:02}:{4:02}'.format(
        year, month, day, hour, minute)


# 创建文件
def __create_file(path: str,
                  name: str,
                  attribute: str = '4',
                  text: str = '') -> str:
    '''
